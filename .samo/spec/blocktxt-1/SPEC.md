...
**Originality pass (v0.2).** Seven visual/layout axes were changed to be deliberately distinct from any specific commercial falling-block implementation. The public mathematical specifications (SRS kicks, 7-bag, Guideline scoring formula) are preserved verbatim; only presentation changes.
...
- **Cell glyphs.** Filled cells render as `▰▰` (U+25B0 "Black Parallelogram") rather than the canonical `██` full-block. The flash-frame glyph used at the start of a line clear is `▣▣` (U+25A3). Monochrome mode keeps per-kind ASCII letters for accessibility.
...
- **Next-piece preview.** The HUD shows exactly ONE upcoming piece (via `GameState::peek_next_kind()`), rendered as the piece's centered ASCII letter inside the bordered "next" box, not as a miniature playfield of cells and not as a queue of multiple lookahead previews. Letter color tracks the current palette's piece accent color.
- **Ghost (landing indicator).** The canonical piece-shaped ghost outline is replaced with a single horizontal `▔▔` floor-line glyph (U+2594) drawn at each column the piece's bottom will occupy at rest, colored at 40% of the active-piece color. The line renders at the top of the landing row so it reads as a crisp floor marker. When the piece is already grounded (ghost_y == active.origin.1) no line is drawn — the piece's own body occupies the landing row.
- **Locked-piece coloration.** Canonical convention keeps a locked cell at the same color as its active-piece state; the originality pass renders locked cells at 60% brightness via `dim_color(theme.color(kind), LOCKED_CELL_DIM)` so the falling piece visibly pops above the settled stack. The active piece and hold/next previews stay at full color; only the board's locked cells are dimmed.

Rationale: all seven changes were driven by a user directive invoking *Tetris Holding v. Xio Interactive, Inc.* (2012, D.N.J.) as precedent for trade-dress enforcement on falling-block presentations. The ruling explicitly named the playfield dimensions, per-piece color associations, and cell visual style (first three changes); the user directive also cited the next-piece preview, ghost-piece style, and locked-piece color convention as additional protected elements warranting deliberate distinction. The trademark-scrub posture (see `scripts/check-naming.sh` + the "Release checklist item" above) is unchanged.
